# NavigationDrawerDestination

- Python class: `NavigationDrawerDestination`
- Source: `sdk/python/packages/flet/src/flet/controls/material/navigation_drawer.py`

## Serialized Properties (Python layer)
- `label`
- `icon`
- `selected_icon`
- `bgcolor`

## Event Fields
- _No `on_*` fields detected._

## Notes
- Extracted from `@control("NavigationDrawerDestination")` class declaration.
- Property list is based on dataclass field declarations in class body.
- Final wire payload is MessagePack-encoded by Python messaging protocol.
