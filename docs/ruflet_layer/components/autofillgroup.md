# AutofillGroup

- Python class: `AutofillGroup`
- Source: `sdk/python/packages/flet/src/flet/controls/core/autofill_group.py`

## Serialized Properties (Python layer)
- `content`
- `dispose_action`

## Event Fields
- _No `on_*` fields detected._

## Notes
- Extracted from `@control("AutofillGroup")` class declaration.
- Property list is based on dataclass field declarations in class body.
- Final wire payload is MessagePack-encoded by Python messaging protocol.
