...
inline std::filesystem::path packaged_project_root() {
  ...
  if (configured != nullptr && configured[0] != '\0') {
    ...
    return std::filesystem::exists(candidate / "main.rb", error) ? candidate
                                                                 : std::filesystem::path();
  }
  ...
  for (const auto &entry : std::filesystem::directory_iterator(root, error)) {
    ...
    if (!std::filesystem::exists(entry.path() / "main.rb", error)) {
      continue;
    }
    if (!found.empty()) {
...
inline void begin(StartFunction start) {
  ...
  const std::string entrypoint = (root / "main.rb").string();
