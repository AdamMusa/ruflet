...
class RufletFilledTonalButtonCompatibilityTest < Minitest::Test
  def test_filled_tonal_button_accepts_positional_content_and_serializes_current_flet_props
    button = Ruflet.filled_tonal_button(
      "Next",
      icon: "add",
      icon_color: "#ABCDEF",
      color: "#FEDCBA",
    ...
    assert_equal "Next", patch["content"]
  ...
  def test_filled_tonal_button_defaults_match_flet_button
    button = Ruflet.filled_tonal_button("Next")
  ...
  def test_compact_alias_uses_same_control
    assert_equal "FilledTonalButton", Ruflet.filledtonalbutton("Next").to_patch["_c"]
  end
end
