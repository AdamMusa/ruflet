...
class RufletFilledTonalIconButtonCompatibilityTest < Minitest::Test
  def test_filled_tonal_icon_button_accepts_positional_icon_and_serializes_current_flet_props
    button = Ruflet.filled_tonal_icon_button(
      "check",
      alignment: { x: 0, y: 0 },
      bgcolor: "#ABCDEF",
    ...
    assert_equal "FilledTonalIconButton", patch["_c"]
    assert_equal Ruflet::MaterialIconLookup.codepoint_for("check"), patch["icon"]
    ...
    assert_equal Ruflet::MaterialIconLookup.codepoint_for("close"), patch["selected_icon"]
  ...
  def test_filled_tonal_icon_button_defaults_match_flet
    button = Ruflet.filled_tonal_icon_button("check")
  ...
  def test_filled_tonal_icon_button_serializes_non_positive_splash_radius_like_flet
    assert_equal 0, Ruflet.filled_tonal_icon_button("check", splash_radius: 0).to_patch["splash_radius"]
    assert_equal(-1, Ruflet.filled_tonal_icon_button("check", splash_radius: -1).to_patch["splash_radius"])
  end

  def test_compact_alias_uses_same_control
    assert_equal "FilledTonalIconButton", Ruflet.filledtonaliconbutton("check").to_patch["_c"]
  end
end
