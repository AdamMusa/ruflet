...
class RufletPageCompatibilityTest < Minitest::Test
  ...
  def test_page_supports_theme_animation_and_views_pop_until_api
    page = Ruflet::Page.new(
      session_id: "s1",
      client_details: { "route" => "/" },
      sender: ->(_action, _payload) {}
  ...
  def test_page_accepts_ruflet_chrome_assignments
    sent = []
    page = Ruflet::Page.new(
      session_id: "s1",
      client_details: { "route" => "/" },
  ...
  def test_page_exposes_view_properties
    sent = []
    page = Ruflet::Page.new(
      session_id: "s1",
      client_details: { "route" => "/" },
  ...
  def test_page_overlay_behaves_as_a_list_property
    sent = []
    page = Ruflet::Page.new(
      session_id: "s1",
      client_details: { "route" => "/" },
  ...
  def test_page_get_control_and_schedule_update
    sent = []
    page = Ruflet::Page.new(
      session_id: "s1",
      client_details: { "route" => "/" },
